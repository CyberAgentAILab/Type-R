...
class CrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0):
        ...
        self.to_out = zero_module(
            nn.Sequential(nn.Linear(inner_dim, query_dim), nn.Dropout(dropout))
        )
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, t_context=None, v_context=None):
        ...
        if hasattr(self, "t_attn"):
            x = self.t_attn(self.t_norm(x), context=t_context) + x
        if hasattr(self, "v_attn"):
            x = self.v_attn(self.v_norm(x), context=v_context) + x
