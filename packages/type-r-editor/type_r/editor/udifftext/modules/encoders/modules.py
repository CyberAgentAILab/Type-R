...
class FrozenOpenCLIPImageEmbedder(AbstractEmbModel):
    ...
    def preprocess(self, x):
        ...
        # )
        x = F.resize(img=x, size=(224, 224), interpolation=T.InterpolationMode.BICUBIC)
        x = (x + 1.0) / 2.0
...
class ViTSTREncoder(VisionTransformer):
    ...
    def __init__(self, size=224, ckpt_path=None, freeze=True, *args, **kwargs):
        ...
        self.resize = transforms.Resize(
            (size, size), transforms.InterpolationMode.BICUBIC, antialias=True
        )
...
class LabelEncoder(AbstractEmbModel, pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.pos_embedding = PositionalEncoding(
            d_model=self.emd_dim, max_len=self.max_len
        )
        transformer_block = nn.TransformerEncoderLayer(
            d_model=self.emd_dim, nhead=self.n_heads, batch_first=True
        )
        self.encoder = nn.TransformerEncoder(
            transformer_block, num_layers=self.n_trans_layers
        )
        ...
        if trainable:
            ...
            self.visual_head = nn.Sequential(
                *[
                    ...
                    nn.Conv1d(
                        in_channels=self.visual_len, out_channels=1, kernel_size=1
                    ),
                ]
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        clip_loss, cls_loss, pos_loss, logits_per_text = self.get_loss(
            text_out, visual_out, clip_target, cls_out, pos_out, cls_target, pos_target
        )
