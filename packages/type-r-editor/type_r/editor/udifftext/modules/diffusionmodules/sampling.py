...
DEFAULT_GUIDER = {
    "target": "type_r.editor.udifftext.modules.diffusionmodules.guiders.IdentityGuider"
}
...
class BaseDiffusionSampler:
    ...
    def denoise(self, x, model, sigma, cond, uc):
        denoised = model.denoiser(
            model.model, *self.guider.prepare_inputs(x, sigma, cond, uc)
        )
...
class EulerEDMSampler(EDMSampler):
    ...
    def attend_and_excite(
        self, x, model, sigma, cond, batch, alpha, iter_enabled, thres, max_iter=20
    ):
        # calc timestep
        ...
        )  # https://github.com/yuval-alaluf/Attend-and-Excite/blob/main/pipeline_attend_and_excite.py#L288

        iters = 0
        while True:
            model_output = model.model(x, c_noise, cond)
            local_loss = model.loss_fn.get_min_local_loss(
                model.model.diffusion_model.attn_map_cache,
                batch["mask"],
                batch["seg_mask"],
            )
            grad = torch.autograd.grad(
                local_loss.requires_grad_(True), [x], retain_graph=True
            )[0]
            x = x - alpha * grad
            iters += 1
    ...
    def sampler_step(
        ...
        if update:
            x = self.attend_and_excite(
                x, model, sigma_hat, cond, batch, alpha, iter_enabled, thres
            )
        ...
        if save_attn:
            attn_map = model.model.diffusion_model.save_attn_map(
                save_name=name, tokens=batch["label"][0]
            )
...
class DPMPP2MSampler(BaseDiffusionSampler):
    ...
    def __call__(
        self, denoiser, x, cond, uc=None, num_steps=None, init_step=0, **kwargs
    ):
        x, s_in, sigmas, num_sigmas, cond, uc = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
