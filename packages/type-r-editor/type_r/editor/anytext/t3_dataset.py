...
def get_caption_pos(ori_caption, pos_idxs, prob=1.0, place_holder="*"):
    ...
    for i in range(len(pos_idxs)):
        if random.random() < prob and pos_idxs[i] > 0:
            pos += (
                place_holder
                + random.choice([" located", " placed", " positioned", ""])
                + random.choice([" at", " in", " on"])
                + idx2pos[pos_idxs[i]]
                + ", "
            )
        else:
    ...
    def load_data(self, json_path, percent):
        ...
        for gt in content["data_list"]:
            if len(d) > max_img:
                break
            if (
                "wm_score" in gt and gt["wm_score"] > self.wm_thresh
            ):  # wm_score > thresh will be skiped as an img with watermark
                wm_skip += 1
                continue
            data_root = content["data_root"]
            data_root = data_root.replace(
                "/data/vdb/yuxiang.tyx/AIGC/data", self.data_dir, 1
            )
            # if self.using_dlc:
            #     data_root = data_root.replace(
            #         "/data/vdb/yuxiang.tyx/AIGC/data", self.data_dir, 1
            ...
            d.append(info)
        print(
            f"{json_path} loaded, imgs={len(d)}, wm_skip={wm_skip}, time={(time.time() - tic):.2f}s"
        )
        if count > 0:
            print(
                f"Found {count} image's caption contain placeholder: {self.place_holder}, change to ' '..."
            )
        return d
    ...
    def __getitem__(self, item):
        ...
        if len(texts) > 0:
            ...
            item_dict["caption"] = get_caption_pos(
                item_dict["caption"], pos_idxs, self.caption_pos_porb, self.place_holder
            )
            item_dict["polygons"] = [cur_item["polygons"][i] for i in sel_idxs]
            item_dict["texts"] = [
                cur_item["texts"][i][: self.max_chars] for i in sel_idxs
            ]
            ...
            for idx, text in enumerate(item_dict["texts"]):
                ...
                glyphs = draw_glyph2(
                    self.font, text, item_dict["polygons"][idx], scale=self.glyph_scale
                )
        ...
        # inv_mask
        invalid_polygons = (
            cur_item["invalid_polygons"] if "invalid_polygons" in cur_item else []
        )
        if len(texts) > 0:
        ...
        if n_pad > 0:
            item_dict["glyphs"] += [
                np.zeros((512 * self.glyph_scale, 512 * self.glyph_scale, 1))
            ] * n_pad
...
if __name__ == "__main__":
    ...
    for i, data in enumerate(train_loader):
        ...
        masked_img = ((data["masked_img"][0].numpy() + 1.0) / 2.0 * 255)[
            ..., ::-1
        ].astype(np.uint8)
        ...
        cv2.imwrite(
            os.path.join(show_imgs_dir, f"plots_{i}.jpg"), np.array(img)[..., ::-1]
        )
        ...
        for k, glyphs in enumerate(data["glyphs"]):
            cv2.imwrite(
                os.path.join(show_imgs_dir, f"plots_{i}_glyph_{k}.jpg"),
                glyphs[0].numpy().astype(np.int32) * 255,
            )
            all_glyphs += [glyphs[0].numpy().astype(np.int32) * 255]
        cv2.imwrite(
            os.path.join(show_imgs_dir, f"plots_{i}_allglyphs.jpg"),
            np.sum(all_glyphs, axis=0),
        )
        for k, gly_line in enumerate(data["gly_line"]):
            cv2.imwrite(
                os.path.join(show_imgs_dir, f"plots_{i}_gly_line_{k}.jpg"),
                gly_line[0].numpy().astype(np.int32) * 255,
            )
        for k, position in enumerate(data["positions"]):
            if position is not None:
                cv2.imwrite(
                    os.path.join(show_imgs_dir, f"plots_{i}_pos_{k}.jpg"),
                    position[0].numpy().astype(np.int32) * 255,
                )
        cv2.imwrite(
            os.path.join(show_imgs_dir, f"plots_{i}_hint.jpg"),
            data["hint"][0].numpy().astype(np.int32) * 255,
        )
        cv2.imwrite(
            os.path.join(show_imgs_dir, f"plots_{i}_inv_mask.jpg"),
            np.array(img)[..., ::-1]
            * (1 - data["inv_mask"][0].numpy().astype(np.int32)),
        )
        pbar.update(1)
    pbar.close()
