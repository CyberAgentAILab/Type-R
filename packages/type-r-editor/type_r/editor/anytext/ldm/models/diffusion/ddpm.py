...
from ..autoencoder import AutoencoderKL, IdentityFirstStage
...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if True:  # block grad in invalid mask areas
            ...
            inv_mask = torch.nn.functional.interpolate(inv_mask, size=(64, 64)).repeat(
                1, 4, 1, 1
            )
        ...
        if self.loss_alpha > 0 or self.loss_beta > 0:
            ...
            step_weight = extract_into_tensor(
                self.alphas_cumprod, t, x_start.shape
            ).reshape(len(t))
            ...
            origin_x0 = (
                (cond["text_info"]["img"] + 1.0) / 2.0 * 255
            )  # -1,1 -> 0,255; n,h,w,c
            origin_x0 = rearrange(origin_x0, "n h w c -> n c h w")
            if PRINT_DEBUG:
                cv2.imwrite(
                    "00.origin.jpg",
                    origin_x0[0].permute(1, 2, 0).detach().cpu().numpy()[..., ::-1],
                )
                cv2.imwrite(
                    "00.decode.jpg",
                    decode_x0[0].permute(1, 2, 0).detach().cpu().numpy()[..., ::-1],
                )
            ...
            for i in range(bsz):
                ...
                for j in range(n_lines):  # line
                    ...
                    pos = cond["text_info"]["positions"][j][i] * 255.0
            ...
            if len(x0_texts) > 0:
                ...
                for i in range(bsz):
                    if len(bs_preds_neck_decode[i]) > 0:
                        if self.loss_alpha > 0:
                            ...
                            ocr_loss_debug += (
                                sp_ocr_loss.detach().cpu().numpy().tolist()
                            )
                        else:
                        ...
                        if self.loss_beta > 0:
                            ...
                            ctc_loss_debug += (
                                sp_ctc_loss.detach().cpu().numpy().tolist()
                            )
                        else:
                ...
                if PRINT_DEBUG:
                    print(
                        f"loss_ocr: {loss_ocr.mean().detach().cpu().numpy():.4f}, loss_ctc: {loss_ctc.mean().detach().cpu().numpy():.4f}, loss_simple: {loss_simple.mean().detach().cpu().numpy():.4f}, Weight: loss_alpha={self.loss_alpha}, loss_beta={self.loss_beta}, step_weight={step_weight.detach().cpu().numpy()}, latin_weight={self.latin_weight}"
                    )
