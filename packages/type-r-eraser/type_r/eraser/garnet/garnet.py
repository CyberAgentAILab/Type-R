...
class MaskSpatialGateAttention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        att1 = self.conv(out)
        att2 = self.conv2(out)
        return self.sig3(self.alpha * att1 + self.beta * att2), (
            self.sig(att1),
            self.sig2(att2),
        )
...
class Residual_with_Attention(nn.Module):
    def __init__(
        self, in_channels, out_channels, dilation=1, same_shape=True, Guide=True
    ):
...
class GaRNet(nn.Module):
    def __init__(self, in_channels):
        ...
        self.uplayer1 = nn.Sequential(
            nn.ConvTranspose2d(2, ndf * 4, kernel_size=4, stride=2, padding=1),
            nn.ELU(alpha=1.0),
        )
        ...
        self.convs_1 = nn.Conv2d(
            ndf, in_channels, kernel_size=1, stride=1, padding=0, bias=False
        )
        ...
        self.convs_2 = nn.Conv2d(
            ndf, in_channels, kernel_size=1, stride=1, padding=0, bias=False
        )

        self.uplayer5 = nn.Sequential(
            nn.ConvTranspose2d(ndf, in_channels, kernel_size=4, stride=2, padding=1),
            nn.ELU(alpha=1.0),
        )
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=6, ndf=64, num_blocks=3):
        ...
        self.initialblock = nn.Sequential(
            nn.Conv2d(
                in_channels, ndf, kernel_size=kernel_size, stride=2, padding=padding
            ),
...
class label_Discriminator(nn.Module):
    def __init__(
        self, in_channels, ndf=1, n_layers=3, use_sigmoid=False, use_bias=False
    ):
