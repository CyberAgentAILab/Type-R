...
from ...clip4str.data.utils import (
    BaseTokenizer,
    CharsetAdapter,
    CTCTokenizer,
    Tokenizer,
)
from ...clip4str.models.post_process import clip_post_process
...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(
        self, batch, validation: bool, clip_refine: bool = False, **kwargs
        ...
        if clip_refine:
            ...
            for i, prob in enumerate(batch_probs):
                post_prob, post_ids, post_pred = clip_post_process(
                    self.clip_model,
                    images[i, ...].unsqueeze(0),
                    prob,
                    self.charset_adapter,
                    self.tokenizer,
                    K=5 if "sample_K" not in kwargs else kwargs["sample_K"],
                    K2=0 if "sample_K2" not in kwargs else kwargs["sample_K2"],
                    num_samples=50
                    if "sample_total" not in kwargs
                    else kwargs["sample_total"],
                    prompt=None
                    if "sample_prompt" not in kwargs
                    else kwargs["sample_prompt"],
                    alpha=0.1 if "alpha" not in kwargs else kwargs["alpha"],
                )
    ...
    def test_step(
        self, batch, batch_idx, clip_refine: bool = False, **kwargs
    ) -> Optional[STEP_OUTPUT]:
        if clip_refine and self.clip_model is None:
            print("[CLIP] creating CLIP model for post refinement")
            model, clip_transform = clip.load(
                kwargs["clip_model_path"], device=batch[0].device
            )
