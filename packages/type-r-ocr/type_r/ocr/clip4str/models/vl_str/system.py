...
from ....clip4str.clip import clip
from ....clip4str.models.base import CrossEntropySystem

from .modules import Decoder, DecoderLayer, modify_attn_mask
...
class VL4STR(CrossEntropySystem):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.image_freeze_nlayer = (
            kwargs["image_freeze_nlayer"]
            if "image_freeze_nlayer" in kwargs.keys()
            else -1
        )
        self.text_freeze_nlayer = (
            kwargs["text_freeze_nlayer"]
            if "text_freeze_nlayer" in kwargs.keys()
            else -1
        )
        ...
        self.use_language_model = (
            kwargs["use_language_model"]
            if "use_language_model" in kwargs.keys()
            else False
        )
        self.context_length = (
            kwargs["context_length"] if "context_length" in kwargs.keys() else 20
        )
        self.cross_loss_w = (
            kwargs["cross_loss_w"] if "cross_loss_w" in kwargs.keys() else 1.0
        )
        self.use_share_dim = (
            kwargs["use_share_dim"] if "use_share_dim" in kwargs.keys() else True
        )
        self.cross_gt_context = (
            kwargs["cross_gt_context"] if "cross_gt_context" in kwargs.keys() else True
        )
        self.cross_cloze_mask = (
            kwargs["cross_cloze_mask"] if "cross_cloze_mask" in kwargs.keys() else False
        )
        self.cross_correct_once = (
            kwargs["cross_correct_once"]
            if "cross_correct_once" in kwargs.keys()
            else False
        )
        self.image_detach = (
            kwargs["image_detach"] if "image_detach" in kwargs.keys() else True
        )
        self.cross_token_embeding = (
            kwargs["cross_token_embeding"]
            if "cross_token_embeding" in kwargs.keys()
            else False
        )
        ...
        rank_zero_info(
            ...
            "\t cross_gt_context: {}, cross_cloze_mask: {}, cross_fast_decode: {} \n".format(
                ...
                self.cross_gt_context,
                self.cross_cloze_mask,
                self.cross_fast_decode,
            )
        )

        assert "clip_pretrained" in kwargs.keys()
        if not os.path.exists(kwargs["clip_pretrained"]):
            kwargs["clip_pretrained"] = os.path.join(
                CLIP_PATH, os.path.basename(kwargs["clip_pretrained"])
            )
        ...
        self.clip_model.transformer.apply(
            lambda m: modify_attn_mask(m, context_length=self.context_length)
        )
        ...
        vis_embed_dim = (
            ...
            if self.use_share_dim
            else self.clip_model.visual.proj.shape[0]
        )
        rank_zero_info(
            "The dimension of the visual decoder is {}.".format(vis_embed_dim)
        )
        decoder_layer = DecoderLayer(
            vis_embed_dim, dec_num_heads, vis_embed_dim * dec_mlp_ratio, dropout
        )
        # We don't predict <bos> nor <pad>, so num_classes=len(self.tokenizer) - 2, label length + 1 for <eos>
        self.visual_decoder = Decoder(
            decoder_layer,
            dec_depth,
            norm=nn.LayerNorm(vis_embed_dim),
        ...
        if self.use_language_model:
            ...
            decoder_layer = DecoderLayer(
                cross_embed_dim, dec_num_heads, cross_embed_dim * dec_mlp_ratio, dropout
            )
            self.cross_decoder = Decoder(
                decoder_layer,
                dec_depth,
                norm=nn.LayerNorm(cross_embed_dim),
    ...
    def encode(self, img: torch.Tensor):
        ...
        if self.freeze_image_backbone:
            with torch.no_grad():
                memory = self.clip_model.encode_image(
                    img, projection=self.use_share_dim
                )
        else:
    ...
    def visual_decode(
        ...
        return self.visual_decoder(
            tgt, memory, tgt_query, tgt_query_mask, content_mask, tgt_padding_mask
        )

    def encoder_cross_modal_feature(self, prev_logits, image_feat):
        prev_logits = prev_logits.detach().clone()
        image_features = (
            image_feat.detach().clone() if self.image_detach else image_feat
        )
        ...
        captions, _ = self.tokenizer.decode_fast(
            probs, charset_adapter=self.charset_adapter
        )
        text = clip.tokenize(
        ...
        if self.freeze_language_backbone:
            with torch.no_grad():
                text_features = (
                    self.clip_model.token_embedding(text)
                    if self.cross_token_embeding
                    else self.clip_model.encode_text(text)
                )
        else:
            text_features = (
                self.clip_model.token_embedding(text)
    ...
    def cross_decode(
        ...
        return self.cross_decoder(
            tgt, cross_memory, tgt_query, tgt_query_mask, content_mask, tgt_padding_mask
        )
    ...
    def forward(self, images: Tensor, max_length: Optional[int] = None) -> Tensor:
        ...
        vis_pos_queries = self.visual_decoder.pos_queries[:, :num_steps].expand(
            bs, -1, -1
        )
        crs_pos_queries = (
            self.cross_decoder.pos_queries[:, :num_steps].expand(bs, -1, -1)
            if self.use_language_model
            else None
        )

        # a left-to-right auto-regressive mask, special case for the forward permutation
        content_mask = query_mask = torch.triu(
        ...
        if self.decode_ar:
            ...
            for i in range(num_steps):
                ...
                p_i, visual_vec = self.visual_decode(
                    tgt_in[:, :j],
                    memory,
            ...
            logits, visual_vec = self.visual_decode(
                bos, memory, tgt_query=vis_pos_queries
            )
        ...
        if self.use_language_model:
            ...
            if self.cross_fast_decode:
                ...
                cross_logits, cross_vec = self.cross_decode(
                    logits,
                ...
                for i in range(crs_num_steps):
                    j = i + 1  # next token index
                    p_i, cross_vec = self.cross_decode(
                        logits,
                        tgt_in[:, :j],
                        memory,
        ...
        if self.refine_iters:
            ...
            for i in range(self.refine_iters):
                ...
                tgt_padding_mask = (tgt_in == self.eos_id).cumsum(
                    -1
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        L = tgt_in.shape[1]
        cloze_content_mask = cloze_query_mask = torch.triu(
            torch.full((L, L), float("-inf"), device=self._device), 1
        )
        cloze_query_mask[
            torch.triu(torch.ones(L, L, dtype=torch.bool, device=self._device), 2)
        ] = 0
        ...
        for i, perm in enumerate(tgt_perms):
            ...
            visual_logits, visual_vec = self.visual_decode(
                tgt_in,
                memory,
            ...
            loss += n * F.cross_entropy(
                visual_logits.flatten(end_dim=1),
                tgt_out.flatten(),
                ignore_index=self.pad_id,
            )
            all_inter_vars.append(
                (visual_logits.detach().clone(), query_mask, content_mask)
            )
            ...
            if self.use_language_model:
                ...
                cross_tgt_in = (
                    tgt_in
                    if self.cross_gt_context
                    else torch.cat([bos, visual_logits[:, :-1].argmax(-1)], dim=1)
                )
                cross_query_mask = (
                    cloze_query_mask if self.cross_cloze_mask else query_mask
                )
                cross_content_mask = (
                    cloze_content_mask if self.cross_cloze_mask else content_mask
                )
                cross_logits, cross_vec = self.cross_decode(
                    visual_logits,
                    cross_tgt_in,
                ...
                loss += (
                    ...
                    * n
                    * F.cross_entropy(
                        cross_logits.flatten(end_dim=1),
                        tgt_out.flatten(),
                        ignore_index=self.pad_id,
                    )
                )
    ...
    def configure_optimizers(self):
        ...
        enc_gain_or_bias_params = [
            p for n, p in encoder_params if exclude(n, p) and p.requires_grad
        ]
        enc_rest_params = [
            p for n, p in encoder_params if include(n, p) and p.requires_grad
        ]

        # decoder parameters
        decoder_params = [
            (n, p) for n, p in list(self.named_parameters()) if "clip_model" not in n
        ]
        dec_gain_or_bias_params = [
            p for n, p in decoder_params if exclude(n, p) and p.requires_grad
        ]
        dec_rest_params = [
            p for n, p in decoder_params if include(n, p) and p.requires_grad
        ]
        ...
        optimizer = torch.optim.AdamW(
            [
                {"params": enc_gain_or_bias_params, "weight_decay": 0.0, "lr": lr},
                {
                    "params": enc_rest_params,
                    "weight_decay": self.weight_decay,
                    "lr": lr,
                },
                {
                    "params": dec_gain_or_bias_params,
                    "weight_decay": 0.0,
                    "lr": lr * self.coef_lr,
                },
                {
                    "params": dec_rest_params,
                    "weight_decay": self.weight_decay * self.coef_wd,
                    "lr": lr * self.coef_lr,
                },
            ],
            lr=lr,
        ...
        sched = OneCycleLR(
            optimizer,
            [lr, lr, lr * self.coef_lr, lr * self.coef_lr],
    ...
    def freeze_cip_layers(
        self,
        image_freeze_nlayer,
        text_freeze_nlayer,
        image_freeze_layer_divisor=-1,
        image_only_fc=False,
    ):
        """
        ...
        """
        assert (
            image_freeze_nlayer <= 12
            and text_freeze_nlayer <= 12
            and image_freeze_layer_divisor <= 12
        )
        if hasattr(self, "clip_model"):
            if image_freeze_nlayer > -1:
                for name, param in self.clip_model.visual.named_parameters():
                    ...
                    if (
                        name.startswith("ln_post.")
                        or name.startswith("proj")
                        or name.startswith("conv1")
                        or name.startswith("ln_pre")
                    ):
                        continue
                    elif name.startswith("transformer.resblocks."):
                        layer_num = int(name.split(".resblocks.")[1].split(".")[0])
