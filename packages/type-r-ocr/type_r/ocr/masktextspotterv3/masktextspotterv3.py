...
from maskrcnn_benchmark.utils.chars import get_tight_rect, getstr_grid
from maskrcnn_benchmark.utils.checkpoint import DetectronCheckpointer
from PIL import Image
from torchvision import transforms as T

logger = logging.getLogger(__name__)
CONFIG_FILE = str(Path(__file__).parent / "default.yaml")
...
class TextDemo(object):
    ...
    def run_on_opencv_image(self, image, proposals=None):
        """
        Arguments:
        ...
        if proposals is None:
            result_polygons, result_words, proposal_items = self.compute_prediction(
                image.astype(np.uint8)
            )
            return result_polygons, result_words, proposal_items
        else:
            result_words = self.compute_prediction_from_proposals(
                image.astype(np.uint8), _proposals=proposals
            )
            return result_words

    def compute_prediction(self, original_image, _proposals=None):
        ...
        # compute predictions
        with torch.no_grad():
            predictions, proposals, _ = self.model(image_list)
        ...
        result_words = []
        proposal_items = []
        # logger.info(f"{len(boxes)=}")
        for k, box in enumerate(boxes):
            score = scores[k]
            # logger.info(f"{score=} {words[k]=}")
            ...
            proposal_box = proposals[0].bbox[k].tolist()
            proposal_polygon = (
                proposals[0].extra_fields["masks"].polygons[k].polygons[0].tolist()
            )
            proposal_items.append((proposal_box, proposal_polygon))
        # logger.info(result_words)
        return result_polygons, result_words, proposal_items

    def compute_prediction_from_proposals(self, original_image, _proposals=None):
        # apply pre-processing to image
        image = self.transforms(original_image)
        # convert to an ImageList, padded so that it is divisible by
        # cfg.DATALOADER.SIZE_DIVISIBILITY
        ...
        # compute predictions
        with torch.no_grad():
            predictions, _, _ = self.model(image_list, _proposals=_proposals)
        global_predictions = predictions[0]
        char_predictions = predictions[1]
        char_mask = char_predictions["char_mask"]
        char_boxes = char_predictions["boxes"]
    ...
    def visualization(self, image, polygons, words):
        for polygon, word in zip(polygons, words):
            ...
            cv2.putText(
                image, word, (xmin, ymin), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--visu_path", type=str, default="./demo_images/demo_results.jpg"
    )
